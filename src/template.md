...
Затем следует расписать каждый элемент: что в нем описывается, как это лучше делать, а также пример оформления этого элемента.
...
Если в документе в большинстве случаев несколько элементов будут повторяться, можно создать шаблон сразу целого блока с несколькими элементами.
Такой блок будет полезен, чтобы скопировать его, вставит к себе в документ и заполнить нужными данными.
...
??? example "Пример шаблона вступительного блока"
    ...
    === "Исходный код"
        ...
        > Бэкенд описан в спецификации [«Название спецификации».](./#elements)
